...
func (m *AICopilotModal) ViewInline(width int) string {
	...
	if m.TelemetrySummary != "" {
		ctxInfo += " " + lipgloss.NewStyle().Foreground(lipgloss.Color("#81C784")).Render("• " + m.TelemetrySummary)
	}
	var histBadge string
	turns := m.GetChatTurns()
	if len(turns) > 0 {
		curr := m.historyNavIndex + 1
		if m.historyNavIndex == -1 {
			curr = 0
		}
		histBadge = lipgloss.NewStyle().Foreground(ColorWarning).Render(fmt.Sprintf(" "+i18n.T("ai_history_badge"), curr, len(turns)))
	}
	escHint := lipgloss.NewStyle().Foreground(ColorMuted).Render("[Esc: " + i18n.T("btn_close") + "]")

	leftHeader := lipgloss.JoinHorizontal(lipgloss.Center, title, " ", modelBadge, ctxInfo, histBadge)
	...
	if m.IsStreaming {
		...
		if m.IsDangerous {
			warnBadge := lipgloss.NewStyle().Bold(true).Foreground(ColorWhite).Background(ColorDanger).Padding(0, 1).Render(i18n.T("ai_warn_dangerous_cmd"))
