...
func (m *Model) renderStatusBar() string {
	...
	if status == "" {
		status = lipgloss.NewStyle().Foreground(ColorMuted).Render(i18n.T("status_ready"))
	} else {
		status = lipgloss.NewStyle().Foreground(ColorSuccess).Bold(true).Render(status)
	}

	line1 := StatusBarStyle.Width(m.width - 1).Render(keys)
	line2 := lipgloss.NewStyle().Background(ColorBg).Padding(0, 1).Width(m.width - 1).Render(status)
	return line1 + "\n" + line2
}
