...
const (
	...
	MetricExtractScript = `cat /proc/stat | grep '^cpu '; sleep 0.1; cat /proc/stat | grep '^cpu '; echo '` + MetricSplitDelimiter + `'; cat /proc/meminfo; echo '` + MetricSplitDelimiter + `'; df -k /; echo '` + MetricSplitDelimiter + `'; cat /proc/net/dev; echo '` + MetricSplitDelimiter + `'; uname -srm; cat /etc/os-release 2>/dev/null | grep PRETTY_NAME; uptime -p 2>/dev/null || cat /proc/uptime 2>/dev/null; nproc 2>/dev/null || grep -c ^processor /proc/cpuinfo 2>/dev/null`
)
...
func ParseRawBundle(rawOutput string) (*RawTelemetryBundle, error) {
	...
	instantCPU, cpuSnap, err := ParseDualProcStat(sections[0])
	if err != nil || cpuSnap == nil {
		cpuSnap, _ = ParseProcStat(sections[0])
	}
