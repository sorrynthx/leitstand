...
### 3. 🛡️ SSH 베스천 터널링 & OpenSSH 호스트 키 검증
- **폐쇄망 프라이빗 서브넷 접근**: 사내 방화벽 내부의 폐쇄망 서버도 표준 SSH ProxyJump 베스천 호스트를 경유하여 원클릭으로 직접 터널링 연결.
- **OpenSSH TOFU 호스트 키 검증 (`~/.ssh/known_hosts`)**: OpenSSH 표준 규격의 TOFU (Trust-On-First-Use) 검증 엔진 탑재. 첫 접속 서버 키 자동 등록 및 호스트 키 위·변조 발생 시 즉시 접속을 차단하여 중간자 공격(MITM) 원천 방어.
